...
if (isset($action) && $action=='delete' && isset($seqno)) {
    // Invalid this item
	$sqlcmd = "SELECT * FROM parking WHERE seqno='$seqno' AND valid='Y'";
	$rs = querydb($sqlcmd, $db_conn);
	if (count($rs) > 0) {
		$sqlcmd = "delete from parking WHERE seqno='$seqno'";
...
$PageTitle = '停車位資訊系統';
$sqlcmd = "SELECT count(*) AS reccount FROM parking";
$rs = querydb($sqlcmd, $db_conn);
...
$sqlcmd = "SELECT * FROM parking LIMIT $StartRec,$ItemPerPage";
...
<html>
	...
	<tr>
	  ...
	  <th width="15%">ParkID</th>
	  <th width="15%">Name</th>
	  <th width="30%">Address</th>
	  <th width="15%">HeightLimit</th>
      <th width="15%">GpsLocation</th>	 
      <th width="15%">NoOfBlocks</th>	 	  
	</tr>
	<?php
	foreach ($Contacts AS $item) {
	  ...
	  $Valid = $item['valid'];
	  $DspMsg = "'確定刪除項目?'";
	  $PassArg = "'parking.php?action=delete&seqno=$seqno'";
	?>
	<tr align="center">
	  <td>
	<?php
	...
	<?php } else { ?>
	  ...
	  作廢</a>&nbsp;
	  <a href="parkingmod.php?seqno=<?php echo $seqno; ?>">
	  修改</a>&nbsp;	  
	  </td>	    
	<?php } ?>
	  <td><?php echo $seqno ?></td>  
	  <td><?php echo $ParkID ?></td>
	  <td><?php echo $Name ?></td>
	  <td><?php echo $Address?></td>
	  <td><?php echo $HeightLimit ?></td>
