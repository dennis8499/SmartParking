...
if (isset($action) && $action=='recover' && isset($seqno)) {
    // Recover this item
    $sqlcmd = "SELECT * FROM parkingblock WHERE seqno='$seqno' AND valid='Y' AND CarState='N'";
    $rs = querydb($sqlcmd, $db_conn);
    if (count($rs) > 0) {
        $sqlcmd = "UPDATE parkingblock SET CarState='Y' WHERE seqno='$seqno'";
...
if (isset($action) && $action=='change' && isset($seqno)) {
    // Invalid this item
	$sqlcmd = "SELECT * FROM parkingblock WHERE seqno='$seqno' AND valid='Y' AND CarState='Y'";
	$rs = querydb($sqlcmd, $db_conn);
	if (count($rs) > 0) {
		$sqlcmd = "UPDATE parkingblock SET CarState='N' WHERE seqno='$seqno'";
		$result = querydb($sqlcmd, $db_conn);
	}
}
...
<html>
	...
	<tr>
	  ...
	  <th width="15%">Time</th>
	  <th width="15%">Checkservation</th>
	  <th width="15%">CarState</th>   	 	  
	</tr>
	<?php
	foreach ($Contacts AS $item) {
      ...
      $Checkservation = $item['checkreservation']; 	  
	  $Valid = $item['valid'];
	  $DspMsg = "'確定刪除項目?'";
	  $DspMsg2 = "'確定停止使用?'";
	...
	<tr align="center">
	  ...
	  <td><?php echo $Time?></td>
	  <td><?php echo $Checkservation ?></td>
      <td><?php echo $CarState?></td> 	  
	</tr>
	<?php
	}
	?>
