...
function mqtt_start()
    m = mqtt.Client("TTU001", 120)
	m:lwt("SmartParking/SensorState", "TTU001".."offline", 2, 0)
     ...
     elseif (State == 1) then         
       ...
       if(device.measure_avg() <= trigger) then  
            if(SendGetSign == 0) then	
                distance = device.measure_avg();		    
                m:publish("SmartParking/TTU001","01" + distance,2,0)                 
                SendGetSign = 1
                SendNotGetSign = 0				
            end     
       elseif(device.measure_avg() > trigger) or (device.measure_avg() == -1) then 
            if (SendNotGetSign == 0 and SendGetSign == 1) then			   
                m:publish("SmartParking/TTU001","02" + distance,2,0)               
                SendGetSign = 0
                SendNotGetSign = 1				
           end     
           State = 0    
