...
function mqtt_register()
    ...
    m:publish("SmartParking/SensorState","TTA_IN".."start",2,0)   
    end)
end
...
function mqtt_start()
    m = mqtt.Client("TTA_IN", 120)
	m:lwt("SmartParking/SensorState", "TTA_IN".."offline", 2, 0)
...
function detect_car()
	 ...
	 if (value >= 0 and value <= trigger) then
	   if(sign == 0) then
	      m:publish("SmartParking/Sensor","#1TTA_IN",2,0)  
		  sign = 1	  
	   end	  
	 else
	   sign = 0	 
