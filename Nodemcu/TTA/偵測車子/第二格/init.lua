...
function mqtt_register()
    ...
    m:publish("SmartParking/SensorState","TTA002".."start",2,0)   
    end)
end
...
function mqtt_start()
    m = mqtt.Client("TTA002", 120)
	m:lwt("SmartParking/SensorState", "TTA002".."offline", 2, 0)
     ...
     elseif (State == 1) then         
       ...
       if(device.measure_avg() <= trigger) then  
            if(SendGetSign == 0) then			    
                m:publish("SmartParking/Sensor","#2".."TTA002",2,0)                 
                SendGetSign = 1
                SendNotGetSign = 0				
            end     
       elseif(device.measure_avg() > trigger) or (device.measure_avg() == -1) then 
            if (SendNotGetSign == 0 and SendGetSign == 1) then			   
                m:publish("SmartParking/Sensor","#3".."TTA002",2,0)               
                SendGetSign = 0
                SendNotGetSign = 1				
           end     
           State = 0    
